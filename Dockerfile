# This dockerfile uses the ubuntu:latest image
# VERSION 0 - EDITION 1
# Author:  Michelot, Thomas, thomas.michelot@epita.fr
# Command format: Instruction [arguments / command] ..

FROM ubuntu:latest
MAINTAINER Michelot, Thomas, thomas.michelot@epita.fr
...
RUN DEBIAN_FRONTEND=noninteractive apt-get install -y gawk wget git-core diffstat unzip texinfo gcc-multilib \
     ...
     xz-utils debianutils iputils-ping vim bc g++-multilib bash manpages-fr-extra

# Graphical and Eclipse Plug-In Extras
RUN DEBIAN_FRONTEND=noninteractive apt-get install -y libsdl1.2-dev xterm

# Documentation
RUN DEBIAN_FRONTEND=noninteractive apt-get install -y make xsltproc docbook-utils fop dblatex xmlto
...
#RUN DEBIAN_FRONTEND=noninteractive apt-get install -y xvfb libtool libncurses5-dev libssl-dev zlib1g-dev:i386 tftpd
